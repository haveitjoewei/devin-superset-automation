...
def test_matches_the_specific_pr_not_the_latest():
    s = _session()
    s.add_all([
        Job(issue_url="https://github.com/haveitjoewei/superset/issues/1", issue_number=1, state="checks_running", pr_number=100),
        Job(issue_url="https://github.com/haveitjoewei/superset/issues/1", issue_number=2, state="checks_running", pr_number=200),  # the "latest"
    ])
    s.commit()
...
def test_no_pr_reference_or_no_match_returns_none():
    s = _session()
    s.add(Job(issue_url="https://github.com/haveitjoewei/superset/issues/1", issue_number=1, state="checks_running", pr_number=100))
    s.commit()
...
def test_only_matches_active_jobs():
    s = _session()
    s.add(Job(issue_url="https://github.com/haveitjoewei/superset/issues/1", issue_number=1, state="merged", pr_number=100))
    s.commit()
